...
class StatisticsController extends Controller
{
    ...
    /**
     * Get state admin dashboard stats
     *
     * @param Request $request
     * @return ApiResponse
     */
    public function state(Request $request) {
        try {
            ...
            $baseQuery = User::role('member', 'api')
                ->where('reg_status', 'done')
                ->whereHas('details', function ($q) use ($state) {
                    $q->where('state', $state);
                });
            $baseCpdLogQuery = CpdLog::where('status', 'approved')
                ->whereYear('created_at', Carbon::now()->year);
    ...
    /**
     * Get state admin's chart data
     * @return ApiResponse
     */
    public function state_charts() {
        try {
            $user = auth()->user();
            $userDetails = $user->details()->first();
            ...
            return ApiResponse::success('State admin dashboard charts fetched successfully', [
    ...
    /**
     * State stats and breakdown
     *
     * @param \Illuminate\Http\Request $request
     * @return ApiResponse
     */
    public function state_breakdown(Request $request) {
        try {
            ...
            return ApiResponse::success('State report charts fetched successfully', [
